...
export function getHttpApiError(error: any): HttpApiError | null {
  if (typeof error === 'object' && error !== null && typeof error.getErrorMessage === 'function') {
    return error as HttpApiError;
  } else {
    return null;
  }
}

export function createHttpApiClient(uri: string, accessToken: string | null = null): HttpApiClient {
  return new AxiosHttpApiClientImpl(uri, accessToken);
}
...
class AxiosHttpApiClientImpl implements HttpApiClient {
  ...
  public constructor(uri: string, accessToken: string | null = null) {
    const instance = axios.create({
      baseURL: `${apiBaseUrl}${uri}`,
    });
    this.instance = this.setInterceptors(instance, accessToken);
  }
  ...
  private setInterceptors(instance: AxiosInstance, accessToken: string | null) {
    // Add a request interceptor
    instance.interceptors.request.use(
      (config: AxiosRequestConfig) => {
        // 인증 사용 시 인증 헤더를 추가하여 전송한다.
        if (accessToken) {
    ...
    instance.interceptors.response.use(
      ...
      async (error: Error | AxiosError<ErrorResponseDto>) => {
        ...
        const httpApiError = new AxiosHttpApiErrorImpl(axios.isAxiosError(error) ? error as AxiosError : new AxiosError<ErrorResponseDto>(error.message));
